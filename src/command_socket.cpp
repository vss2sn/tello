...
void CommandSocket::sendCommand(const std::string& cmd){
  n_retries_ = 0;
  ASYNC_SEND;
  std::this_thread::sleep_for(constants::joystick_timeout); // TODO(vss): reduce this to less than amount of time joystick waits?
}
...
void CommandSocket::waitForResponse(){
  ...
  while(std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now() - start).count() < timeout_ && on_){
    ...
    std::this_thread::sleep_for(constants::waiting_for_response_timeout);// TODO(vss): replace with condition variable
  }
  if(!on_){
    waiting_for_response_ = false;
    return;
  ...
  if(waiting_for_response_) {
    utils_log::LogInfo() << "Timeout - Attempt #" << n_retries_ << " for command [" << last_command_ << "].";
  }
  ...
  while(on_){
    ...
    while(!command_queue_.empty() && execute_queue_){
      ...
      if(cmd.substr(0,5) == "delay"){
        std::this_thread::sleep_for(std::chrono::seconds(stoi(cmd.substr(5, cmd.size()))));
        // usleep(1000000 * stoi(cmd.substr(5, cmd.size())));
        continue;
      }
      else if((cmd.compare(0,4,"stop") == 0) || (cmd.compare(0,9,"emergency") == 0)) {
...
void CommandSocket::doNotAutoLandWorker(){
  ...
  while(on_){
    ...
    if(std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now() - command_sent_time_) > constants::dnal_timeout){
      sendCommand(cmd);
      last_command_ = cmd;
      waiting_for_response_ = false;
    }
    else{
      std::this_thread::sleep_for(constants::dnal_timeout); //10 seconds, command needs to be sent ever 15
    }
  }
...
bool CommandSocket::isExecutingQueue() const {
...
CommandSocket::~CommandSocket(){
  ...
  cv_dnal_.notify_all();
  std::this_thread::sleep_for(constants::minor_pause); // allow dnal thread to exit before setting execute_queue_ to true.
  {
    std::lock_guard<std::mutex> lk(m);
    execute_queue_ = true;
  }
