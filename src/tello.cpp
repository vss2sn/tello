...
#endif
    std::condition_variable &cv_run, const std::string &drone_ip,
    const std::string &local_drone_port, const std::string &local_video_port,
    const std::string &local_state_port, const std::string &camera_config_file,
    const std::string &vocabulary_file, const int n_retries, const int timeout,
    const std::string &load_map_db_path, const std::string &save_map_db_path,
    const std::string &mask_img_path, bool load_map, bool continue_mapping,
    float scale, const std::string &sequence_file)
    : io_service_(io_service), cv_run_(cv_run) {
  cs = std::make_unique<CommandSocket>(io_service, drone_ip, "8889",
                                       local_drone_port, n_retries, timeout);
  vs = std::make_unique<VideoSocket>(
      io_service, "0.0.0.0", "11111", local_video_port, run_,
      camera_config_file, vocabulary_file, load_map_db_path, save_map_db_path,
      mask_img_path, load_map, continue_mapping, scale);
  ss = std::make_unique<StateSocket>(io_service, "0.0.0.0", "8890",
                                     local_state_port);
...
void Tello::terminalToCommandThread() {
  ...
  std::cout << "----------- Terminal to command thread exits -----------"
            << std::endl;
}
    ...
    case BUTTON_START:
      if (js_->getButtonState(BUTTON_LEFT_BUMPER_2) != 0u) {
        ...
        std::this_thread::sleep_for(
            constants::wait_for_land); // Block any other joystick input
        utils_log::LogWarn() << "Exit called from joystick";
        // NOTE: Notification of calling end of code kept here* to allow
        // expansion to swarm, where a single joystick might be used to command
        // multiple Tellos at which point this function will be commented using
        // a #define and a swarm joystick function would be run that sends
        // caommands to all the Tellos in the swarm. it would be rewuired that
        // the above function be called for all the tellos before the
        // cv_run_.notify_all is called.
