...
public:
   ...
   * @param [in] n_retries_allowed numebr of retries allowed if a response is
   * not received from the drone before sending the next command in the
   * execution queue
   * @param [in] timeout number of seconds after which a command is said to have
   * failed to be sent
   * @return none
   */
  ...
  /**
   * @brief will set the bool value that prevents automatic landing due to no
   * commands sent timeout
   * @return void
   */
  void doNotAutoLand();

  /**
   * @brief will unset the bool value that prevents automatic landing due to no
   * commands sent timeout
   * @return void
   */
  void allowAutoLand();
  ...
  /**
   * @brief sends the "emergency" command to the drone that will cause the
   * motors to stop immediately and stops queue exection as well
   * @return void
   */
  void emergency();
