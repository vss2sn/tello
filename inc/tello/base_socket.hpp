...
public:
   ...
   * @return no return
   */
  BaseSocket(asio::io_service &io_service, const std::string &drone_ip,
...
private:
   ...
   * @param [in] error error thrown by socket when receiving a response from
   * drone
  ...
  // NOTE: Passing cmd by ref or as const causes problems dues to async nature
  // of code

  /**
   * @brief function called to send command to drone
   * @param [in] error error thrown by socket when sending a command to the
   * drone
  ...
  virtual void handleSendCommand(const std::error_code &error,
                                 size_t bytes_sent, std::string cmd) = 0;
};
